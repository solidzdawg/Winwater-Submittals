...
Usage:
    ...
    python assemble-submittal.py --project "Double-RR" --validate-only
...
def assemble_submittal(project: str, output_name: str | None = None) -> Path:
    """
    Merge all PDFs in the correct submittal order into one output PDF.

    Order:
      1. 01-cover/
      2. 02-index/
      3. 03-items/Item-XX/   (sorted; files within each folder sorted by name)
      4. 04-attachments/
    ...
    # Cover and index sections
    for section_dir in [project_dir / "01-cover", project_dir / "02-index"]:
        pdfs = find_pdfs_in_dir(section_dir)
        if not pdfs:
            print(f"  ⚠️  No PDFs in {section_dir.name}/ — section skipped")
    ...
    if not item_dirs:
        print("  ⚠️  No item folders in 03-items/ — items section skipped")
    ...
    # Attachments
    for pdf_path in find_pdfs_in_dir(project_dir / "04-attachments"):
        reader = PdfReader(str(pdf_path))
        page_count = len(reader.pages)
        for page in reader.pages:
            writer.add_page(page)
    ...
    if total_pages == 0:
        print("\n⚠️  No pages assembled — check that PDFs exist in the project folders.")
        sys.exit(1)
    ...
    with open(log_path, "w") as f:
        f.write("Winwater Submittal Assembly Log\n")
...
def validate_project_structure(project: str) -> bool:
    """
    Check that all expected directories exist and contain PDFs.
    Returns True if ready to assemble, False if there are blockers.
    """
    project_dir = SUBMITTALS_DIR / project
    required_dirs = ["01-cover", "02-index", "03-items", "04-attachments"]
    all_ok = True
    ...
    for d in required_dirs:
        ...
        if d == "03-items":
            ...
            ok = exists and pdf_count > 0
            status = f"{'✅' if ok else '⚠️ '} {d}/  ({pdf_count} PDF files)"
            if not ok:
                all_ok = False
        print(f"  {status}")
    ...
    if all_ok:
        ...
        print(
            ...
            "Run 'python scripts/agent.py --project <name>' to populate them automatically."
        )
    return all_ok
